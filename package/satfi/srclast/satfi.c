...
static void *select_tsc(void *p)
{
	...
	while(1)
	{
		...
		if(sock_tsc < 0)
		{
			...
			satfi_log("sat_available=%d n3g_status=%d svrconcnt=%d %s %d\n", base->sat.sat_available, base->n3g.n3g_status, svrconcnt, base->tsc.tsc_addr, base->tsc.tsc_port);
			if((sock_tsc = ConnectTSC(base->sat.sat_ifname, base->tsc.tsc_addr, base->tsc.tsc_port, &err, 0)) == -1)
			//if((sock_tsc = ConnectTSC("3g-wan0", base->tsc.tsc_addr, base->tsc.tsc_port, &err, 10)) == -1)
			{
				char ucbuf[256];
				...
				iCntUserSave = 0;
				if(err != ETIMEDOUT) seconds_sleep(5);
			}
			else
			{
				svrconcnt = 0;
