...
int handle_app_msg_tcp(int socket, char *pack)
{
	char tmp[4096] = {0};
	char userid[24] = {0};
	int n = 0;
	...
	switch(p->mclass)
	{
		...
		case MO_VOICE:
		{
			...
			StrToBcd(rsp->MsID, &(req->MsID[USERID_LLEN - USERID_LEN]), USERID_LEN);
			if(datalen > 0)memcpy(rsp->data, req->data, datalen);
			satfi_log("MO_VOICE %s datalen=%d %d %d\n",userid, datalen, rsp->header.length, req->header.length);
						...
						if(nread > 0)
						{
							...
							while(1)
							{
								...
								if((sizeof(pHeader->length) + pApp->DataSaveOffset > nread) || (nread < pHeader->length + pApp->DataSaveOffset))
								{
									...
									memcpy(pApp->Data, pHeader, pApp->DataSaveOffset);
									satfi_log("pApp->DataSaveOffset=%d\n", pApp->DataSaveOffset);
									break;
								}
								
								if(handle_app_msg_tcp(pApp->AppSocketFd, (char*)pHeader) == -1)
								{
									pApp->DataSaveOffset = 0;
									break;
								}
...
void Data_To_MsID(char *MsID, Header *pHeader)
{
	int n;
	USER *pUser = gp_users;
	while(pUser)
	{
		if(strncmp(pUser->userid, MsID, USERID_LLEN) == 0)
		{
			if(pUser->socketfd < 0)
			{
				satfi_log("%.21s lost connect", pUser->userid);
				break;
			}
...
int handle_tsc_msg(char *DataFromTsc, char *DataToApp)
{
	...
	switch(pHeader->mclass)
	{
		...
		case FLAG_NOTIFY://no use
		{
			...
			int n = write(sock_tsc, tmp, req->header.length);
			if(n < 0)
			{
...
static void *select_tsc(void *p)
{
	...
	while(1)
	{
		if(base->sat.sat_available == 0 || bSatNetWorkActive == 0)
		{
			bTscConnected = 0;
			if(sock_tsc > 0)
			{
...
void *SystemServer(void *p)
{
	...
	while(1)
	{
		...
		{	
			if(PackHead != NULL)
			{
				//printsocketbufsize(sock_tsc, "test");
				SendPackToTSC();
			}
		}
...
int ConnectTSC(char* routename, char* ip, int port, int *err, int timeout)
{
    ...
    if(connect(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr)) == -1) 
	{
        ...
        satfi_log("Connect Error:%s errno=%d\n", strerror(errno), errno);   
		close(sockfd);
        return -1;
    }
	else
...
int main_fork(void)
{
	...
	//sat拨号线程,ring检测
	strcpy(base.sat.sat_imei,"357975061012409");//TEST
	...
	while(1)
	{
		...
		switch(select(maxfd+1,&fds,NULL,NULL,&timeout))
		{
			...
			default:
				...
				{
					handle_sat_data(base.sat.sat_fd);
				}
