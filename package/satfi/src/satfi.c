	...
	while(1)
	{
		...
		switch(base->sat.sat_state)
		{
			case SAT_STATE_AT:
				...
				bzero(base->sat.sat_imei, 16);
				bzero(base->sat.sat_imsi, 16);
				counter=0;
			break;
			case SAT_STATE_AT_W:
				satfi_log("func_y:send AT to SAT Module\n");
			...
			case SAT_STATE_IMSI_W:
				...
				if(base->sat.sat_state == SAT_STATE_IMSI)counter=0;
			break;
...
void add_user(char *msid, int socketfd)
{
	USER *pUser = gp_users;
...
int handle_app_msg_tcp(int socket, char *pack, char *tscbuf)
{
	...
	switch(p->mclass)
	{
		...
		case SET_BLOCK_CMD:
		{
			...
			satfi_log("APP_SET_BLOCK_CMD %.21s %d %d\n",req->MsID, socket,rsp->header.length);
			n = Data_To_Tsc(tmp, rsp->header.length);
			if(n<0) satfi_log("write to tsc return error: errno=%d (%s) %d\n", errno, strerror(errno),__LINE__);
		}
		break;
		...
		case EMERGENCY_ALARM_CMD:
		{
			...
			satfi_log("APP_EMERGENCY_ALARM_CMD %.21s %d %d\n",req->MsID, socket,rsp->header.length);
			n = Data_To_Tsc(tmp, rsp->header.length);
			if(n<0) satfi_log("write to tsc return error: errno=%d (%s) %d\n", errno, strerror(errno),__LINE__);
		}
		break;
		...
		case PTT:
		{
			...
			StrToBcd(rsp->MsID, &(req->MsID[USERID_LLEN - USERID_LEN]), USERID_LEN);
			StrToBcd(rsp->GrpID, req->GrpID, USERID_LLEN);
			satfi_log("PTT %.21s %.21s %.21s\n",req->MsID, req->MsID, req->GrpID);
		...
		case MO_VOICE:
		{
			...
			if(res == 0)
			{
				...
				StrToBcd(rsp->MsID, &(req->MsID[USERID_LLEN - USERID_LEN]), USERID_LEN);
				if(datalen > 0)memcpy(rsp->data, req->data, datalen);
				satfi_log("OUT_MO_VOICE %.21s datalen=%d %d %d Reserve=%s\n",req->MsID, datalen, rsp->header.length, req->header.length, Reserve);
		...
		case ASPTT_REQ:
		{
			...
			if(res == 0)
			{
				...
				bzero(&tsc_addr, sizeof(tsc_addr));
				tsc_addr.sin_family = AF_INET;
				tsc_addr.sin_port = htons(UDP_VOICE_SRCPORT);
				tsc_addr.sin_addr.s_addr =	inet_addr(base.tsc.tsc_addr);
				sendto(sock_udp, tmp, 0, 0, (struct sockaddr*)&tsc_addr, sizeof(tsc_addr));	
			}
		}
		break;
...
int handle_tsc_msg(char *DataFromTsc, char *DataToApp)
{
	...
	switch(pHeader->mclass)
	{
		...
		case APP_PTT_IN:
		{
			...
			req->header.length = sizeof(Msg_Ptt_Voice);
			req->header.mclass = PPT_VOICE;
			...
			tsc_addr.sin_port = htons(UDP_VOICE_DSTPORT);
			tsc_addr.sin_addr.s_addr =	inet_addr(base.tsc.tsc_addr);
			sendto(sock_udp, tmp, req->header.length, 0, (struct sockaddr*)&tsc_addr, sizeof(tsc_addr));
		}
		break;
				...
				if(FD_ISSET(sock_udp, &fds))
				{
					...
					if (n>0)
					{
						...
						else
						{
							...
							//sendto(sock_udp, buf, n, 0, (struct sockaddr *)&cliAppAddr, len);
						}
					}
					else if (n == 0)
					{
...
static void *select_tsc(void *p)
{
	...
	while(1)
	{
		if(sock_tsc < 0)
		{
			...
			base->sat.sat_status = 0;
			if((sock_tsc = ConnectTSC(base->sat.sat_ifname, base->tsc.tsc_addr, base->tsc.tsc_port, &err, 0)) == -1)
			{
				//satfi_log("ConnectTSC Error:%s errno=%d\n", strerror(err), err);   
				if(err == ECONNREFUSED)//连接拒绝
				{
				}
				else if(err == ETIMEDOUT)//连接超时
				{
					cnt2++;
					if(cnt2 >= 3)
					{
						//myexec("ifdown wan8", NULL, NULL);
						//base->sat.sat_available = 0;
						cnt2 = 0;
					}
			...
			else
			{
				...
				getsockopt(sock_tsc, IPPROTO_TCP, TCP_MAXSEG, &(base->tsc.mss), &optlen);
				satfi_log("base->tsc.mss=%d",base->tsc.mss);
				cnt2 = 0;
				base->sat.sat_status = 1;
			}
...
int ConnectTSC(char* routename, char* ip, int port, int *err, int timeout)
{
    ...
    if(connect(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr)) == -1) 
	{
        ...
        satfi_log("Connect Error:%s errno=%d sockfd=%d\n", strerror(errno), errno, sockfd);   
		close(sockfd);
        return -1;
    }
	else
...
void handle_gps_data(int gpsfd)
{
	...
	if(n-7-2>0)
	{
		...
		{
			if(buf[5] != '3')return;
			memcpy(&gpsbuf[idx], &buf[7], n-7-2);//(AT+U=3,\r\n)去掉协议头7字节 2个尾部包括逗号 ,剩下中间数据
			idx += n-7-2;
...
int main_fork(void)
{
	...
	//sat拨号线程,ring检测
	//srand(time(0));
	//sprintf(base.sat.sat_imei ,"8%014u" ,rand());
	//strcpy(base.sat.sat_imei,"357975061018620");//TEST
	...
	while(1)
	{
		...
		switch(select(maxfd+1,&fds,NULL,NULL,&timeout))
		{
			...
			default:
				...
				{
					handle_sat_data(base.sat.sat_fd);//卫星模块数据
				}

				if(base.n3g.n3g_fd > 0 && FD_ISSET(base.n3g.n3g_fd, &fds))
				{
					handle_gprs_data(base.n3g.n3g_fd);
				}
				break;
		}
	}
